...
checkpointer = ModelCheckpoint(filepath='../weights/tracemap_jc.h5', verbose=1, save_best_only=True)
...
for i in range(5):
    ...
    model.save_weights('../weights/tracemap_jc'+str(i)+'.h5')
...
#
#checkpointer = ModelCheckpoint(filepath='../weights/tracemap_dir.h5', verbose=1, save_best_only=True)
##model.load_weights('../weights/tracemap_dir.h5')
#for i in range(5):
...
#                   validation_steps=200, verbose=1, callbacks=[checkpointer])
#    model2.save_weights('../weights/tracemap_dir'+str(i)+'.h5')